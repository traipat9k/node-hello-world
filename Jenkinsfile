    ...
    stages{
		...
		*/
	    ...
	    stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "/usr/bin/curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'http://192.168.40.140:8080/job/Node101-CD/buildWithParameters?token=gitops-token'"
                }
            }
       }
    }
